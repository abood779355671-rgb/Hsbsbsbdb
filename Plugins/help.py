...
# ─────────────────────────────────────────────────────────────────────────
# cache للصفحات الثابتة: المفتاح = (rank_level, section_key)  TTL = 5 دقائق
# ─────────────────────────────────────────────────────────────────────────
_PAGE_CACHE: dict[tuple, tuple] = {}
_PAGE_CACHE_TTL = 300

def _page_cache_get(level: int, section_key: str):
    entry = _PAGE_CACHE.get((level, section_key))
    if entry and time.monotonic() - entry[2] < _PAGE_CACHE_TTL:
        return entry[0], entry[1]
    return None

def _page_cache_set(level: int, section_key: str, text: str, markup):
    if len(_PAGE_CACHE) >= 100:
        oldest = min(_PAGE_CACHE, key=lambda k: _PAGE_CACHE[k][2])
        _PAGE_CACHE.pop(oldest, None)
    _PAGE_CACHE[(level, section_key)] = (text, markup, time.monotonic())


async def _build_page(uid: int, cid: int, section_key: str, k: str) -> tuple:
    level = _rank_level(uid, cid)
    rank  = await get_rank(uid, cid)  # كانت تستدعي await داخل def عادية — bug إصلاح
    name  = botname()
    ...
    # ── cache: نص الصفحة ثابت لكل مستوى+قسم — خزّنه وأعد استخدامه ──────
    cached = _page_cache_get(level, section_key)
    if cached:
        # الكيبورد يحتوي uid في callback_data لذا يُعاد بناؤه دائماً
        base_text, _ = cached
        nav = []
        for sk in available:
            sv = SECTIONS[sk]
        ...
        return base_text.replace("__RANK__", rank), InlineKeyboardMarkup(rows)

    # ── بناء النص ────────────────────────────────────────────────
    lines = [
        f"<b>✦ {name} — {sec['emoji']} أوامر {sec['title']}</b>",
        f"<i>رتبتك: __RANK__</i>",
        "━" * 20,
        "",
    ]
    ...
    # حفظ النص الأساسي في الـ cache (بدون الرتبة الشخصية)
    _page_cache_set(level, section_key, base_text, None)
...
_HELP_PATTERN = r"^(الاوامر|اوامر|المساعدة|مساعدة|help)$"

@Client.on_message(filters.text & filters.group & filters.regex(_HELP_PATTERN), group=50)
async def help_handler(c: Client, m: Message):
    ...
    k = botkey()
    # resolve_text للتحقق من الأوامر المخصصة (مثل اسم بديل)
    text = resolve_text(m.text, cid)
    if text not in ("الاوامر", "اوامر", "المساعدة", "مساعدة", "help"):
        return

    content, markup = await _build_page(uid, cid, "general", k)
    await m.reply(
        content,
...
async def help_callback(c: Client, query: CallbackQuery):
    ...
    content, markup = await _build_page(uid, cid, section_key, k)
    try:
